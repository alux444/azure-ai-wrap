...
export async function callAgent(agentPrompt: string, userString: string): Promise<string> {
...
export async function runExistingAgent(agentId: string, userString: string): Promise<string> {
  const client = new AgentsClient(projectEndpoint, new DefaultAzureCredential());
...
export async function callAgentWithImage(agentPrompt: string, userString: string, temp: string): Promise<string> {
  const client = new AgentsClient(projectEndpoint, new DefaultAzureCredential());
  ...
  console.log(`Created message with image, message ID: ${message.id}`);
